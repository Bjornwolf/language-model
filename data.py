...
def build_stream(files, config):
    ...
    if 'minibatches' in config:
        ...
        stream = Mapping(Padding(batch), switch_first_two_axes)
    return stream
...
def build_datasets(config):
    ...
    train_stream = build_stream(train_files, config)
    if 'valid_path' in config:
        valid_files = scan_files(config['valid_path'])
        valid_stream = build_stream(valid_files, config)
    else:
        valid_stream = None
